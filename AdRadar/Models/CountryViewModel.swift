...
class CountryViewModel: ObservableObject {
    ...
    func fetchCountryData() async {
        guard let currentToken = accessToken else {
            ...
            self.emptyStateMessage = "Please sign in to view your country data"
            self.error = nil
        ...
        case .success(let countries):
            ...
            // Check if no countries were returned
            if countries.isEmpty {
                self.showEmptyState = true
                self.emptyStateMessage = "No country data available for the selected time period. Try a different date range or ensure your ads are running."
                self.error = nil
            }
        case .failure(let error):
            await handleError(error)
    ...
    private func handleError(_ error: AdSenseError) async {
        ...
        case .unauthorized:
            ...
            if let authViewModel = authViewModel {
                ...
                if refreshSuccess, let newToken = authViewModel.accessToken {
                    ...
                    showEmptyState = true
                    emptyStateMessage = "Please sign in again to access your country data."
                    self.error = nil
                }
            } else {
                showEmptyState = true
        ...
        case .requestFailed(let message):
            ...
            // Check if this is a scope issue
                ...
                self.error = "Country access requires additional permissions. Please grant AdSense access in your Google account settings or contact support."
            } else {
                showEmptyState = true
                emptyStateMessage = "Unable to load country data. Please try again later."
                self.error = nil
            }
        case .noAccountID:
            showEmptyState = true
