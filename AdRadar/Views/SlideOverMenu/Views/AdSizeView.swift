    ...
    private var mainContent: some View {
        Group {
            if viewModel.isLoading {
                ...
                AdSizeEmptyStateView(message: viewModel.emptyStateMessage ?? "No ad size data available for the selected time period. Try a different date range or ensure your ads are running.")
...
#Preview {
    AdSizeView(showSlideOverMenu: .constant(false), selectedTab: .constant(0))
        .environmentObject(AuthViewModel())
}

// MARK: - Ad Size Empty State View

struct AdSizeEmptyStateView: View {
    let message: String
    @Environment(\.colorScheme) private var colorScheme
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            // Enhanced content
            VStack(spacing: 12) {
                Text("No Ad Size Data Available")
                    .soraHeadline()
                    .foregroundColor(.primary)
                    .multilineTextAlignment(.center)
                    .padding(.horizontal, 8)
                
                Text("No ad size data available for the selected time period. Try a different date range or ensure your ads are running.")
                    .soraCaption()
                    .foregroundColor(.secondary)
                    .multilineTextAlignment(.center)
            }
            ...
            ZStack {
                ...
                LinearGradient(
                    gradient: Gradient(stops: [
                        .init(color: Color.purple.opacity(0.08), location: 0),
                        .init(color: Color.purple.opacity(0.04), location: 0.5),
                        .init(color: Color.purple.opacity(0.02), location: 1)
                    ]),
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                )
                ...
                PatternOverlay(color: .purple.opacity(0.03))
            }
        )
        .clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous))
        .shadow(color: Color.purple.opacity(colorScheme == .dark ? 0.2 : 0.1), radius: 16, x: 0, y: 8)
        .overlay(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .stroke(
                    LinearGradient(
